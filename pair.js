...
router.get('/', async (req, res) => {
    ...
    if (!phone.isValid()) {
        if (!res.headersSent) {
            return res.status(400).send({ code: 'Invalid phone number. Please enter your full international number (e.g., 2348109263390 for Nigeria etc.) without + or spaces.' });
        }
        return;
    }
        ...
        try {
            ...
            EliteProTech.ev.on('connection.update', async (update) => {
                ...
                if (connection === 'open') {
                    ...
                    try {
                        const sessionKnight = fs.readFileSync(dirs + '/creds.json', 'utf8'); // read as string
                        
                        // Send session text content as text
...
└─────────────────┈ ⳹\n\n`
            ...
            if (!EliteProTech.authState.creds.registered) {
                ...
                try {
                    let code = await EliteProTech.requestPairingCode(num);
